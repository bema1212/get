  ...
  try {
    ...
    if (response0.ok && response1.ok && response2.ok) {
      ...
      if (response3.ok && response4.ok) {
        ...
        // Create a map of data4Properties by identificatie
        const propertiesMap = new Map();
        data4Properties.forEach(item => {
          propertiesMap.set(item.identificatie, item);
        });

        // Merge additionalData with data4Properties based on identificatie
        const mergedData = additionalData.map(({ identificatie, data, error }) => {
          const originalProperties = propertiesMap.get(identificatie) || {};
          return {
            identificatie,
            ...originalProperties,
