export default async function handler(req, res) { 
  try {
    ...
    const apiUrl7 = `https://pico.geodan.nl/cgi-bin/qgis_mapserv.fcgi?DPI=120&map=/usr/lib/cgi-bin/projects/gebouw_woningtype.qgs&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetFeatureInfo&CRS=EPSG%3A28992&WIDTH=937&HEIGHT=842&LAYERS=gebouw&STYLES=&FORMAT=image%2Fjpeg&QUERY_LAYERS=gebouw&INFO_FORMAT=text/plain&I=611&J=469&FEATURE_COUNT=10&bbox=${target3}`;
    ...
    const [data0, data1Initial, data2, data3, data5] = await Promise.all([
      ...
      fetchWithErrorHandling(apiUrl3, { headers: { 'Content-Type': 'application/json' } }),
