export default async function handler(req, res) {
  try {
    ...
    // Fetch data from all APIs with error handling for each
    const [response0, response1, response2, response5] = await Promise.all([
      ...
      }).catch(() => ({ ok: false })),  // Catch error and ensure the promise resolves
    ...
    if (response1.ok) {
      ...
      data1 = "none"; // If the response is not OK, set EPON to "none"
    }
